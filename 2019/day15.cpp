...
int paint(OpList code, OpList inputs)
{
	...
	int initial = 0;
	std::vector<Point> neighbors;
	std::vector<int64_t> steps = {direc};
	std::vector<Point> steps2 = {cur};

	do {
		if (j == 0) {
			initial = mx.inputs.size();
		}
		...
		if (mx.outputs.size() > 0) {
			...
			std::cout << "|i==================================================================== " << std::endl
			<< "|XXOUT os:" << mx.outputs.size() << " is:" << mx.inputs.size() << " o: " << lout <<std::endl;
			mx.outputs.clear();
			// 4 = wall
			// 9 = droid
			// 6 = visited
			// 8 = droid + oxygen
			if (j > 100000) manualMode = true;

			if (lout == 0) {
				// same position because wall
				Point w = where(cur, direc);
				image[w.x][w.y] = 4;
				steps.pop_back();
				//steps.push_back(direc+10);

				if (!manualMode) {
				/*
					if (image[cur.x+1][cur.y] == 0) {
						direc = 4;
					} else if (image[cur.x][cur.y+1] == 0) {
						direc = 2;
					} else if (image[cur.x][cur.y-1] == 0) {
						direc = 1;
					} else if (image[cur.x-1][cur.y] == 0) {
						direc = 3;
					} else {
					...
					*/
					...
					/*
						...
						if (iSecret % 2 == 0) {
							direc = turnr(direc);
							if (direc == 1 && iSecret % 10 == 0) direc = 2;
							if (direc == 3 && iSecret % 10 == 0) direc = 4;
						} else {
							direc = turnl(direc);
						}
						*/
					} else {
						std::cout << "|Neigh " << neighbors.size() << " " << direc << std::endl;
						Point tmp = neighbors.back();
						neighbors.pop_back();
						if (tmp.x == cur.x + 1) {
							direc = 4;
						} else if (tmp.x == cur.x - 1) {
							direc = 3;
						} else if (tmp.y == cur.y - 1) {
							direc = 1;
						} else {
							direc = 2;
						}
						std::cout << "|Neigh going " << direc << " " << tmp.x << "/" << tmp.y << std::endl;
						//okneighbors.clear();
					}
				} else {
					direc = manual0(image, cur, last, direc, mx);
				}
				...
				Point foo = where(cur, direc);
				if (image[foo.x][foo.y] == 6 || image[foo.x][foo.y] == 4) steps.push_back(direc+10); //pop_back();
				cur = where(cur, direc);
				image[last.x][last.y]  = 6;
				bool uniq = true;
				for (auto it = steps2.rbegin(); it != steps2.rend(); ++it) {
					if (it->x == cur.x && it->y == cur.y) {
						uniq = false;
					}
				}
				if (uniq) steps2.push_back(cur);
				image[cur.x][cur.y] = 9;
				std::cout << "|i set last to 6 :" << last.x << "/" << last.y << " now: " << cur.x << "/" << cur.y << std::endl;
				if (!manualMode) {
					if (neighbors.size() == 0) {
						neighbors = getne(cur, image);
					}
					if (neighbors.size() > 0) {
						std::cout << "|NeighS " << neighbors.size() << " " << direc << std::endl;
						Point tmp = neighbors.back();
						neighbors.pop_back();
						if (tmp.x == cur.x + 1) {
							direc = 4;
						...
						std::cout << "|NeighS going " << direc << " " << tmp.x << "/" << tmp.y << std::endl;
						neighbors.clear();
					} else {
						// just continue walking
					}
				} else {
					direc = manual1(image, cur, last, direc, mx);
				}
			} else {
				std::cout << "ERROR " << lout << std::endl;
			}
			last = cur;
			show(image);
			showpos(cur, last, direc);
			...
			++j;
			mx.inputs.push_back(direc);
			steps.push_back(direc);
			ppl(steps, "XS ", " ", true);
	for (auto it = steps2.begin(); it != steps2.end(); ++it) {
		std::cout << " " << it->x << "/" << it->y;
	}
			std::cout << std::endl;
		}
		...
		++i;
	} while(j < 10100 &&  mx.position < mx.op.size()-1 && mx.status != 10 && mx.status != 1);

	//show(image, score);
	std::cout << "LEN " << steps.size() - 2 << std::endl;
	ppl(steps, " ", " ", true);
	std::cout << "LEN " << steps2.size()  << std::endl;
