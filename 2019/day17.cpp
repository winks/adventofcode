...
struct Point {
	...
	void ppp(std::string prefix) { if (!DEBUG) return; std::cout << pp(prefix) << std::endl;}
};
...
void show(int64_t image[100][100], bool part2 = false)
{
	int minw = MINW;
	int maxw = MAXW;
	int minh = MINH;
...
int paint(OpList code, OpList inputs)
{
	...
	do {
		...
		if (mx.outputs.size() > 0) {
			if (DEBUG) std::cout << "XX " << mx.outputs.size() << std::endl;
		}
		if (mx.outputs.size() > 0) {
	...
	show(image, mx.op[0] == 2);
	if (mx.op[0] != 2) return 1;
	...
	for (auto it = path.begin(); it != path.end(); ++it) {
		Point p = it->first;
		if (DEBUG) std::cout << "Path: " << p.pp() << " " << pd(it->second) << std::endl;
	}
	...
	for (auto it = path.begin()+1; ;) {
		...
		a.first.ppp("a is: ");
		b.first.ppp("b is: ");
		if (DEBUG) std::cout << std::to_string(a.second) << " " << std::to_string(b.second) <<  " " << pdiff(a.first, b.first) << std::endl;
		std::string tmp;
		if (a.second != pdiff(a.first, b.first)) {
			if (DEBUG) std::cout << " dirchange" << std::endl;
			if (forward > 0) {
				rinputs.push_back(std::to_string(forward));
				if (DEBUG) std::cout << " fwd ended:"<< rinput << std::endl;
				forward = 0;
			}
			...
			++forward;
			if (DEBUG) std::cout << "dc: cmd done:"<< rinput << std::endl;
			...
			// go forward
			++forward;
			if (DEBUG) std::cout << "fwd is now :"<< std::to_string(forward) << std::endl;
			a = b;
			++it;
			continue;
		}
	...
	for (auto it = rinputs.begin(); it != rinputs.end(); ++it) {
		if (DEBUG) std::cout << "LINE " << *it << std::endl;
	...
	if (DEBUG) std::cout << "FOO " << rinputs.size() << std::endl;
